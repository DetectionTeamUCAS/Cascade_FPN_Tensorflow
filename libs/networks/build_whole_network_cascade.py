...
from libs.detection_oprations.proposal_target_layer_cascade_ import proposal_target_layer
...
class DetectionNetwork(object):
    ...
    def postprocess_fastrcnn(self, rois, bbox_ppred, scores, img_shape):
        ...
        with tf.name_scope('postprocess_fastrcnn'):
            ...
            bbox_ppred = tf.reshape(bbox_ppred, [-1, 4])
            ...
            for i in range(1, cfgs.CLASS_NUM + 1):
                ...
                tmp_decoded_boxes = encode_and_decode.decode_boxes(encoded_boxes=bbox_ppred,
    ...
    def build_whole_detection_network(self, input_img_batch, gtboxes_batch):
        ...
        for i in range(len(fg_thresholds)):
            if self.is_training:
                rois, bbox_pred, cls_prob, loss_dict = self.cascade_rcnn(rois, gtboxes_batch,
                                                                         input_img_batch,
                                                                         P_list, img_shape, fg_thresholds[i],
                                                                         fpn_box_pred, fpn_bbox_targets,
                                                                         fpn_cls_score,
                                                                         fpn_labels, stage=i + 1)
                ...
                rois, bbox_pred, cls_prob = self.cascade_rcnn(rois, gtboxes_batch, input_img_batch,
                                                              P_list,
        ...
        final_bbox, final_scores, final_category = self.postprocess_fastrcnn(rois=cascade_rois[-1],
